...
const knex = require('knex')({
  // Your knex configuration here
  client: 'pg', // assuming PostgreSQL
  // other settings like connection
});
...
router.put('/:id', [
  ...
  try {
    ...
    // Prepare update object with safe column mapping
    const updates = {};
    
    // Handle basic fields with safe mapping to DB columns
    ...
    // Use knex for safe parametric updates
...
router.delete('/:id', [
  ...
  try {
    ...
    if (!hardDelete) {
      ...
      const hasRelatedData = await knex('messages')
  ...
  try {
    ...
    if (criteria.name) {
      query = query.where(function() {
        this.whereILike('first_name', `%${criteria.name}%`)
            .orWhereILike('last_name', `%${criteria.name}%`);
      });
    }
    
    if (criteria.roleId) {
      query = query.where('role_id', criteria.roleId);
    }
